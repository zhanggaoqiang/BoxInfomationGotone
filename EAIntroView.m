...
#pragma clang diagnostic pop
        ...
        UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, self.frame.size.height - page.titlePositionY + (windowContentHeight>500?0:46.0f), self.scrollView.frame.size.width - 20, titleHeight)];
...
- (void)buildFooterView {
    self.pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, self.frame.size.height - self.pageControlY - (windowContentHeight > 500.f)*32.0f, self.frame.size.width, 20)];
    [self.pageControl setAutoresizingMask:UIViewAutoresizingFlexibleWidth];
    ...
    self.pageControl.pageIndicatorTintColor = COLOR_HEX_RGB(0xc8c8c8);
    self.pageControl.currentPageIndicatorTintColor = COLOR_HEX_RGB(0x4c646a);
    [self addSubview:self.pageControl];
    
    self.skipButton = [[UIButton alloc] initWithFrame:CGRectMake((windowContentWidth - 139.0)/2, self.pageControl.frame.origin.y - ((windowContentHeight < 500)?100.0f:137.0f), 139, 32)];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    self.skipButton.alpha = fmaxf(scrollView.contentOffset.x - windowContentWidth, 0)/(windowContentWidth/2.0f);
}
...
- (void)setPageControlY:(CGFloat)pageControlY {
    ...
    [self.pageControl setFrame:CGRectMake(0, self.frame.size.height - pageControlY - 32.0f, self.frame.size.width, 20)];
}
